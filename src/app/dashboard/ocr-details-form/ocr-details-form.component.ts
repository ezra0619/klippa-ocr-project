...
import { NgAuthService, User } from 'src/app/shared/ng-auth.service.ts.service';
...
export class OcrDetailsFormComponent implements OnInit {
  ...
  constructor(private http: HttpClient, 
                      private readonly afDatabase: AngularFirestore, 
                      public ngAuthService: NgAuthService, 
                      ) {


    // var template = "Financial Document (.png, .jpg, .pdf)";
    // var pdfTextExtractionMethod = "Fast";
    // var url = "https://cdn-kiosk-api.telegraaf.nl/fa8c45b4-77e5-11e8-8f40-d53bd066a070.jpg";

    // const formData = new FormData();
    // formData.append('template', template);
    // formData.append('pdfTextExtractionMethod', pdfTextExtractionMethod);
    // formData.append('url', url);
    // console.log(formData.get("pdfTextExtractionMethod"));
    ...
    // var secondCollection = afDatabase.firestore.collection('users').doc('IDXfLrL7EBZkSuiEeZeQWIqOVdo1')
    //             .collection('scannedDocs').doc();

    // secondCollection.set({
    //   SomeInfo: "haha"
    // })

    // if(ngAuthService.afAuth.currentUser) {
    //   console.log(ngAuthService.userState);
    // }
    // var user = ngAuthService.userState;
    // const userRef: AngularFirestoreDocument<any> = this.afDatabase.doc(`users/${user.uid}`);

    // var currentUser = JSON.parse(localStorage.getItem('user'));
    // var currentUserID = currentUser.uid;

    // ngAuthService.afAuth.onAuthStateChanged(function(user) {
    //   if (user) {
    //     // User is signed in.
